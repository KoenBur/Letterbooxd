...
async function loadListDetail(listId) {
  ...
  const readCount = list.books.filter(b => {
    // rough match by title
    return Object.values(state.readBooks).some(rb => rb.title.toLowerCase() === b.title.toLowerCase());
  }).length;
  ...
  document.getElementById('list-detail-content').innerHTML = `
    ...
    <div style="max-width:1200px;margin:0 auto;padding:32px 20px 60px">
      <div id="list-detail-books" style="display:flex;flex-direction:column;gap:1px">
        ${list.books.map((b, i) => `
          <div class="book-list-item list-book-row" data-title="${escHtml(b.title)}" data-author="${escHtml(b.author)}" style="cursor:pointer">
            <div style="font-size:13px;color:var(--text-muted);width:28px;flex-shrink:0;text-align:right">${i+1}</div>
            <div class="book-list-cover-placeholder" id="list-cover-${i}">
              <svg width="16" height="22" viewBox="0 0 24 32" fill="none"><rect x="0" y="0" width="24" height="32" rx="2" fill="#3a4555"/></svg>
            </div>
            <div class="book-list-info">
              <div class="book-list-title">${escHtml(b.title)}</div>
              <div class="book-list-author">${escHtml(b.author)}</div>
            </div>
            <div id="list-read-badge-${i}" style="font-size:12px;color:var(--text-muted)">
              ${Object.values(state.readBooks).some(rb => rb.title.toLowerCase() === b.title.toLowerCase())
                ? '<span style="color:var(--accent-green);font-weight:600">✓ Read</span>'
                : ''}
            </div>
          </div>
        `).join('')}
  ...
  // Fetch covers and bind click events asynchronously
